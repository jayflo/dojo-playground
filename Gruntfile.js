	...
	grunt.initConfig({
		dojo: {
			dist: {
				options: {
					...
					profile: path.join('profiles', 'sandbox.profile.js'),
		...
		stylus: {
			compile: {
				...
				files: {
					'src/sandbox/resources/main.css': 'src/sandbox/resources/main.styl'
				}
			}
		},
		watch: {
			stylus: {
				files: 'src/sandbox/resources/**/*.styl',
		...
		intern: {
			local: {
				options: {
					runType: 'client',
					config: 'src/sandbox/tests/intern'
				}
			},
			remote: {
				options: {
